...
public:
    ...
    vector<float> getDescriptors(int strategy = 0) {
        ...
        if(strategy == 0) {
            ...
            descriptor.resize(n_features * 2 + 1);
            for (int j = 0; j < n_features; j++)
                descriptor[j] = first_frame[j];
            for (int j = 0; j < n_features; j++)
                descriptor[j + n_features] = mean_frame[j];
            descriptor[n_features * 2] = descriptors.size();
            return descriptor;
        }
    ...
    // Simply play functionality... this could be expanded into a granular engine
    ...
    bool willFinish(int bufferSize) {
        return position + bufferSize >= audio.size();
    }
    
private:
    bool should_loop, is_finished;
    float position;
    vector<float> audio;
